...
void showCurrentUserData()
{
    ...
    if(!g_currentUserFriendList.empty())
    {
        ...
        {
            cout <<"好友"<< user.getId() << " " << user.getName() << " " << user.getState() << endl;
        }
    }
        ...
        if(responsejs.contains("groups"))
        {
            ...
            for(string &groupstr:vec2)
            {
                ...
                for(string &userstr:vec3)
                {
                    ...
                    guser.setId(user_js["id"].get<int>());
                    guser.setName(user_js["name"]);
                    guser.setState(user_js["state"]);
                    guser.setRole(user_js["role"]);
